...
Перед реальным запуском стоит доработать:

- очередь задач вместо простого `asyncio.create_task`;
- лимиты размера репозитория и ZIP-архива;
- sandbox для обработки недоверенного кода;
- очистку временных директорий;
- rate limiting;
- retry policy для LLM API;
- хранение секретов вне репозитория;
- тесты для анализатора зависимостей и repo pipeline;
